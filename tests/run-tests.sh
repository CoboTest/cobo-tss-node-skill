...
# Create a mock systemctl/launchctl to avoid touching real services
create_mock_systemctl() {
  local bin_dir="$1"
  mkdir -p "$bin_dir"

  # Mock systemctl — logs calls to a file, never touches real systemd
...
# Helper: run a script with mock HOME + mock systemctl/launchctl in PATH
run_with_mock_env() {
...
# Test: generates valid systemd unit with mock systemctl
test_install_service_linux_content() {
  ...
  output=$(run_with_mock_env "$fake_home" "$SCRIPT_DIR/install-service.sh" linux --dir "$d")

  svc_file="$fake_home/.config/systemd/user/cobo-tss-node.service"
  if [[ ! -f "$svc_file" ]]; then
...
# Test: install-service.sh linux calls daemon-reload and enable
test_install_service_linux_calls_systemctl() {
  ...
  run_with_mock_env "$fake_home" "$SCRIPT_DIR/install-service.sh" linux --dir "$d" >/dev/null
  ...
  if [[ "$checks" -eq 2 ]]; then
    ...
    log_fail "systemctl calls" "only $checks/2 calls found in log"
  fi
}

# Test: install-service.sh linux with --env flag sets correct start command
test_install_service_linux_env_flag() {
  local d="$TEST_DIR/svc-linux-env"
  local fake_home="$TEST_DIR/svc-linux-env-home"
  setup_test_dir "$d"
  mkdir -p "$fake_home/.config/systemd/user"

  run_with_mock_env "$fake_home" "$SCRIPT_DIR/install-service.sh" linux --dir "$d" --env sandbox >/dev/null

  svc_file="$fake_home/.config/systemd/user/cobo-tss-node.service"
  if grep -q "\-\-sandbox" "$svc_file"; then
    log_pass "install-service respects --env sandbox"
  else
    log_fail "env flag" "--sandbox not found in service file"
  fi
}
...
test_install_service_macos_content() {
  ...
  output=$(run_with_mock_env "$fake_home" "$SCRIPT_DIR/install-service.sh" macos --dir "$d")

  plist="$fake_home/Library/LaunchAgents/com.cobo.tss-node.plist"
  if [[ ! -f "$plist" ]]; then
...
# Test: node-ctl start/stop/restart/status with mock systemctl
test_ctl_start_stop_restart() {
  ...
  for cmd in start stop restart status; do
    ...
    PATH="$mock_bin:$PATH" MOCK_SYSTEMCTL_LOG="$mock_log" \
      bash "$SCRIPT_DIR/node-ctl.sh" "$cmd" --dir "$d" 2>&1 >/dev/null || true
    if grep -q "systemctl.*$cmd\|systemctl.*cobo-tss-node" "$mock_log" 2>/dev/null; then
      log_pass "node-ctl $cmd calls systemctl"
    else
      log_fail "node-ctl $cmd" "no systemctl call logged"
    fi
  done
}

# Test: node-ctl uninstall removes service file + calls disable
test_ctl_uninstall_linux() {
  ...
  # First install the service file
  mkdir -p "$fake_home/.config/systemd/user"
  HOME="$fake_home" PATH="$mock_bin:$PATH" MOCK_SYSTEMCTL_LOG="$mock_log" \
    bash "$SCRIPT_DIR/install-service.sh" linux --dir "$d" 2>&1 >/dev/null
  ...
  > "$mock_log"
  HOME="$fake_home" PATH="$mock_bin:$PATH" MOCK_SYSTEMCTL_LOG="$mock_log" \
    bash "$SCRIPT_DIR/node-ctl.sh" uninstall --dir "$d" 2>&1 >/dev/null

  checks=0
  # Service file should be removed
  [[ ! -f "$svc_file" ]] && checks=$((checks + 1))
  # Should have called disable + daemon-reload
  ...
  if [[ "$checks" -eq 4 ]]; then
    log_pass "uninstall removes service file, calls disable+daemon-reload, keeps data"
  else
    log_fail "uninstall linux" "only $checks/4 checks passed"
  fi
}

# Test: uninstall preserves all data files
test_ctl_uninstall_preserves_data() {
  ...
  if [[ "$checks" -eq 4 ]]; then
    ...
    log_fail "uninstall data" "only $checks/4 data files remain"
  fi
}
